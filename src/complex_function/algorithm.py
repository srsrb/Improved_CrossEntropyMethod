...
def CEM(test_function, EXPERIMENT, CEMi, centroid, seed, sigma, noise_multiplier, max_epochs, pop_size, elites_nb):
    ...
    for epoch in range(max_epochs):
        print(f'Simulating {epoch} / {max_epochs} generations \n', end='\r')
        sys.stdout.write("\033[F")
        # The scores are initialized
        scores = np.zeros(pop_size)
        # The params of policies at iteration t+1 are drawn according to a multivariate
        # Gaussian whhich center is `centroid`` and which shape is defined by cov
        ...
        # -> fonction_update_covariance(elite_weights) au lieu du bloc
        if EXPERIMENT:
        ...
        # update store weights, centroid and cov matrix
        all_weights.append(
            np.array([t.detach().cpu().numpy() for t in weights]))
...
def CEM_combine(test_function, centroid, seed, sigma, noise_multiplier, max_epochs, pop_size, elites_nb):
    ...
    all_covs = [matrix_CEM.get_cov().detach().cpu().numpy()] + \
        [matrix_CEMir.get_cov().detach().cpu().numpy()]
    ...
    all_weights, all_centroids, all_covs, percentage_CEM, percentage_CEMir = np.array(all_weights), np.array(
        all_centroids), np.array(all_covs), np.array(percentage_CEM), np.array(percentage_CEMir)
...
def CEM_combine_2(test_function, centroid, seed, sigma, noise_multiplier, max_epochs, pop_size, elites_nb):
    ...
    all_covs = [matrix_CEM.get_cov().detach().cpu().numpy()] + [matrix_CEMir.get_cov(
    ).detach().cpu().numpy()] + [matrix_CEMi.get_cov().detach().cpu().numpy()]
    for epoch in range(max_epochs):
        print(f'Simulating {epoch} \n', end='\r')
        sys.stdout.write("\033[F")
        # The scores are initialized
    ...
    all_weights, all_centroids, all_covs = np.array(
        all_weights), np.array(all_centroids), np.array(all_covs)
